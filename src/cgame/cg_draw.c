...
void CG_Text_PaintChar( float x, float y, float width, float height, float scale, float s, float t, float s2, float t2, qhandle_t hShader, scralign_t align ) {
	...
	w = width * scale;
	h = height * scale;
	CG_AdjustFrom640( &x, &y, &w, &h, align );
...
//	if(0) {
	...
	aw = 48;
	ah = 48;
	CG_AdjustFrom640( &ax, &ay, &aw, &ah, ALIGN_BOTTOMRIGHT );
...
static void CG_DrawWeapReticle( void ) {
	...
	CG_AdjustFrom640( &x, &y, &w, &h, ALIGN_CENTER );
	// Knightmare added
	if (cgs.screenXScale > cgs.screenYScale)
	{
		sideWidth = 0.5 * ((cgs.glconfig.vidWidth - (cgs.screenAvgScale * 480)) / cgs.screenAvgScale);
	}
	...
	if ( weap == WP_SNIPERRIFLE ) {
		// sides
		CG_FillRect (0, 0, sideWidth, 480, color, ALIGN_LEFT);
		CG_FillRect (640-sideWidth, 0, sideWidth, 480, color, ALIGN_RIGHT);
...
//----(SA)	end
		...
		if ( cgs.media.snooperShaderSimple ) {
			CG_DrawPic( 80, 0, 480, 480, cgs.media.snooperShaderSimple, ALIGN_CENTER );
		}
...
static void CG_DrawBinocReticle( void ) {
	...
	if (cgs.screenXScale > cgs.screenYScale)
	{
		float sideWidth = 1 + 0.5 * ((cgs.glconfig.vidWidth - (cgs.screenAvgScale * 640)) / cgs.screenAvgScale);
		CG_FillRect (0, 0, sideWidth, 480, color, ALIGN_LEFT);
		CG_FillRect (640-sideWidth, 0, sideWidth, 480, color, ALIGN_RIGHT);
	}
...
//	CG_AdjustFrom640( &x, &y, &w, &h, ALIGN_STRETCH );	
...
//		CG_FillRect( -10, -10, 650, 490, col );
		CG_FillRect( 0, 0, 640, 480, col, ALIGN_STRETCH ); // why do a bunch of these extend outside 640x480?
	}
}
...
//			Vector4Set( color, 0.5f, 0.3f, 0.5f, 1.0f - frac );
		...
		CG_FillRect( -10, -10, 650, 490, color, ALIGN_STRETCH );
	}
}
...
static void CG_DrawFlashDamage( void ) {
	...
	if ( cg.v_dmg_time > cg.time ) {
		...
		CG_FillRect( -10, -10, 650, 490, col, ALIGN_STRETCH );
	}
}
...
static void CG_DrawFlashFire( void ) {
	...
	if ( alpha > 0 ) {
		...
		col[3] = alpha;
		trap_R_SetColor( col );
		CG_DrawPic( -10, -10, 650, 490, cgs.media.viewFlashFire[( cg.time / 50 ) % 16], ALIGN_STRETCH );
		trap_R_SetColor( NULL );
...
static void CG_DrawFlashLightning( void ) {
	...
	if ( alpha > 0 ) {
		...
		CG_DrawPic( -10, -10, 650, 490, shader, ALIGN_STRETCH );
	}
}
...
static void CG_DrawGameScreenFade( void ) {
	...
	VectorClear( col );
	col[3] = cg.viewFade;
	CG_FillRect( 0, 0, 640, 480, col, ALIGN_STRETCH );
}
...
static void CG_ScreenFade( void ) {
	...
	if ( msec <= 0 ) {
		...
		CG_FillRect( 0, 0, 640, 480, cg.fadeColor1, ALIGN_STRETCH );
		...
		if ( color[ 3 ] ) {
			CG_FillRect( 0, 0, 640, 480, color, ALIGN_STRETCH );
		}
	}
}
