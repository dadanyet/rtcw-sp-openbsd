...
void AdjustFrom640( float *x, float *y, float *w, float *h, scralign_t align ) {
	...
	case ALIGN_CENTER:
		if (x) {
			tmp_x = *x;
			*x = (tmp_x - (0.5 * SCREEN_WIDTH)) * DC->avgscale + (0.5 * DC->glconfig.vidWidth);
		}
		if (y) {
			tmp_y = *y;
			*y = (tmp_y - (0.5 * SCREEN_HEIGHT)) * DC->avgscale + (0.5 * DC->glconfig.vidHeight);
		}
		if (w) 
			*w *= DC->avgscale;
		if (h)
	...
	case ALIGN_LETTERBOX:
		...
		if ( w != NULL && h != NULL && ((float)DC->glconfig.vidWidth / (float)DC->glconfig.vidHeight > *w / *h) ) {
			...
			if (x != NULL && w != NULL) {
				tmp_x = *x;
				tmp_w = *w;
				*x = tmp_x * DC->xscale - (0.5 * (tmp_w * vertscale - tmp_w * DC->xscale));
			}
			if (y)
				*y = 0;
			if (w) 
			...
			if (y != NULL && h != NULL) {
				tmp_y = *y;
				tmp_h = *h;
				*y = tmp_y * DC->yscale - (0.5 * (tmp_h * DC->xscale - tmp_h * DC->yscale));
			}
			if (w) 
				*w *= DC->xscale;
			if (h)
	...
	case ALIGN_BOTTOM:
		...
		if (y) {
			tmp_y = *y;
			*y = (tmp_y - SCREEN_HEIGHT) * DC->avgscale + DC->glconfig.vidHeight;
		}
		break;
	case ALIGN_RIGHT:
		if (w) 
		...
		if (x) {
			tmp_x = *x;
			*x = (tmp_x - SCREEN_WIDTH) * DC->avgscale + DC->glconfig.vidWidth;
		}
		if (y) {
			tmp_y = *y;
			*y = (tmp_y - (0.5 * SCREEN_HEIGHT)) * DC->avgscale + (0.5 * DC->glconfig.vidHeight);
...
void Menu_Paint( menuDef_t *menu, qboolean forcePaint ) {
	...
	if ( (DC->xscale > DC->yscale) && (!Q_strcasecmp(menu->window.name, "connect") || !Q_strcasecmp(menu->window.name, "briefing") || !Q_strcasecmp(menu->window.name, "pregame")))
	{
		...
		DC->setColor( color );
		DC->drawHandlePic( 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, menu->window.background, ALIGN_STRETCH );
		DC->setColor( NULL );
	}
